...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		//sorted from largest to smallest, with repetitions allowed
		List<Integer> degree = new ArrayList<Integer>(numVertices) ; 
		
		for(int i = 0 ; i < numVertices ; i++) {
			
			int v = i ;
			int toNeighbors = getNeighbors(v).size() ;
			int fromNeighbors = getInNeighbors(v).size() ;
			
			degree.add(toNeighbors + fromNeighbors) ;
		}
